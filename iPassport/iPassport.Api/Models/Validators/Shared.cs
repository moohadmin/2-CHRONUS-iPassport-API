...
namespace iPassport.Api.Models.Validators
{
    ...
    public class RgValidator : AbstractValidator<string>
    {
        ...
        {
            RuleFor(x => x)
               .Length(1,15).WithMessage($"O campo {fieldName} não é um RG Valido")
               .Must(y => Regex.IsMatch(y, "^[0-9a-zA-Z]+$"))
               .WithMessage($"O campo {fieldName} não é um RG Valido");
        }
    }
    ...
    public class PassportValidator : AbstractValidator<string>
    {
        ...
        {
            RuleFor(x => x)
               .Length(3,15).WithMessage($"O campo {fieldName} não é um Passaporte Valido")
               .Must(y => Regex.IsMatch(y, "^[a-zA-Z]{2}[0-9]+$")).WithMessage($"O campo {fieldName} não é um Passaporte Valido");
        }
    }
