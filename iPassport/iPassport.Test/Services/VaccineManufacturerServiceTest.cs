...
namespace iPassport.Test.Services
{
    ...
    public class VaccineManufacturerServiceTest
    {
        ...
        {
            var mockFilter = Mock.Of<GetByNamePartsPagedFilter>();

            // Arrange
            _mockRepository.Setup(r => r.GetByNameInitals(It.IsAny<GetByNamePartsPagedFilter>()).Result).Returns(new PagedData<VaccineManufacturer>());

            // Act
            var result = _service.GetByNameInitals(mockFilter);

            // Assert
            _mockRepository.Verify(a => a.GetByNameInitals(It.IsAny<GetByNamePartsPagedFilter>()), Times.Once);
